...
struct node *num_passengers_ON_FLOOR_BACK[FLOORS];
...
void insert(int curr_floor,int passenger_ID)
{
  ...
  //Insert at the end
    ...
    while(num_passengers_ON_FLOOR[curr_floor]->next != NULL){
      tmp  = num_passengers_ON_FLOOR[curr_floor]->next;
    }
    tmp->next = new;
  }
}
...
void scheduler_init() {
  ...
  */
  ...
  for(int i = 0; i < PASSENGERS; ++i) {
    ...
    passengers[i].id = -1;
    //passengers[i].to_floor
    //passengers[i].from_floor
  ...
  for(int i = 0; i < FLOORS; ++i) {
    num_passengers_ON_FLOOR_BACK[i] = NULL;
  }
        ...
        void(*exit)(int, int))
{
    ...
    pthread_barrier_wait(&passengers[passenger].barrier_p);
        ...
        */
        if(elevators[passengers[passenger].in_elevator].current_floor == to_floor && elevators[passengers[passenger].in_elevator].state == ELEVATOR_OPEN) {
...
void elevator_ready(int elevator, int at_floor,
    ...
    //pthread_barrier_wait(&elevators[i].barrier_p);
    ///Iterate through elevator array
    //while(1){
      //if(elevator != 0) return; ///Test ith elevator
      ...
      */
          ...
          if(elevators[elevator].occupancy == 1){
            move_direction(elevator,passengers[elevators[elevator].passenger].to_floor - elevators[elevator].current_floor);//Move elevator
            elevators[elevator].current_floor = passengers[elevators[elevator].passenger].to_floor;
          }
          else{
            /*Check bounds if okay to move */
            //GO UP
            if(at_floor == 0 && elevators[elevator].direction == -1)
              elevators[elevator].direction = 1;
            //GO DOWN
            else if(at_floor == (FLOORS - 1) && elevators[elevator].direction == 1)
              elevators[elevator].direction = -1;

            move_direction(elevator,elevators[elevator].direction);//Move elevator
            //Adjust bookeeping of elevator floor
          ...
          */
          ...
          if(elevators[elevator].occupancy == 0 && num_passengers_ON_FLOOR[elevators[elevator].current_floor] != NULL){
            ...
            /*
            //NOT NEEDED
            if(elevators[elevator].passenger == -1){
              elevators[elevator].direction = 1;
              move_direction(elevator,elevators[elevator].direction);//Move elevator
              //Adjust bookeeping of elevator floor
              elevators[elevator].current_floor = at_floor + elevators[elevator].direction;
            ...
            //TESTING Purposes of removeFirstNode() function
            */

            //modifiy passenger inside elevator
            passengers[elevators[elevator].passenger].in_elevator = elevator;//Will adjust variably later
